...
$stmtInfo = $conn->prepare("
    SELECT b.id, c.name as court_name, u.email, u.full_name
    FROM bookings b
...
$stmt = $conn->prepare(
    "UPDATE bookings SET payment_status = 'Paid', payment_verified_at = NOW(), status = 'Confirmed' WHERE id = ?"
);
$stmt->bind_param("i", $booking_id);

if ($stmt->execute()) {
    sendPaymentVerified($info['email'], $info['full_name'], $info);
